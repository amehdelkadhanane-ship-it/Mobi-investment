...
Put your Paystack secret key ONLY in .env on the server.
Put your OpenAI key ONLY in .env on the server.
Set SITE_URL and CORS_ORIGIN to your real domain.
Test Paystack in test mode first, then replace keys with live keys.
Use HTTPS.
Back up data/store.db and uploads/.
Use only product images you have permission to use commercially. The seeded images are for the build/catalog reference and should be replaced with your own supplier/manufacturer/licensed assets before launch if required.
Run locally
Requires Node.js 20+.
npm install
cp .env.example .env
...
AI
The customer AI endpoint is /api/ai/chat. The admin AI endpoint is /api/admin/ai.
